...
export default class Search {
    ...
    set categoryNames(cn) {
      this._categoryNames = cn;
      return cn;
    }
  ...
  `;
      ...
      if (results.length) {
        ...
        this.searchResults.innerHTML = this.categories.map((category) => {
          return results[category].length ?
            `
          <li role="group" aria-labelledby="searchResults-${category}">
            <span class="spectrum-Menu-sectionHeading" id="searchResults-${category}" aria-hidden="true">${this.categoryNames[category]}</span>
