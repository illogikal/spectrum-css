 ...
 */
const isVarFunc = (node) => Boolean(node && node.type === 'function' && node.value === 'var');
 ...
 */
module.exports = function (content, bucketConfig, ret = undefined, { from } = {}) {
    if (!content) return;
     ...
     * @type TokenMetadata
     */
    ret = ret ?? [...bucketConfig.keys()].reduce((acc, key) => ({ ...acc, [key]: new Map() }), {});
    ...
    const addToBucket = (token, settings = {}) => {
        ...
        for (const args of argsArray) {
            ...
            const shouldUpdate = Object.entries(args).map(([k, v]) => {
        ...
        rule.walkDecls((decl) => {
            ...
            valueParser(decl.value).walk((node) => {
                ...
                if (node.nodes.length > 1) {
                    const foundNode = node.nodes.slice(1)?.find((n) => n.type === 'word' || isVarFunc(n));
